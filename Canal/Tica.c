    ...
    if (right->boats[0].priority > left->boats[0].priority) {
       while (counterR < right->count) {
            if (list->head->available == 1) {
                // thread_data_list[TicacounterR].boat = &right->boats[counterR];
        ...
        while (counterL < left->count) {
            if (list->head->available == 1) {
                ...
                // pthread_create(&threalist[counterL], NULL, thread_functionL, (void*)&thread_data_list[counterL]);                counterL++;
            }
        }
    } else {
        while (counterL < left->count) {
